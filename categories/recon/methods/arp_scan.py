...
class ARPScan:
    ...
    def arp_scan(target_ip, timeout, retries, filename, ftype, silent, max_workers=50):
        ...
        if not silent:
            Output.print_arp_banner()

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            results = executor.map(lambda ip: ARPScan.arp_request_ip(ip,timeout,retries), network.hosts())

        active_ips = []
        for hostname, ip, active, mac in results:
            status = "ACTIVE" if active else "INACTIVE"
            if not silent:
                Output.print_arp_result(hostname, ip, status)
            active_ips.append({
                "hostname": hostname,
                "ip": ip,
                "mac": mac,
