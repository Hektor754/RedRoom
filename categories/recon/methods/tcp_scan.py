...
def tcp_connect_scan(network, ports, max_workers=MAX_WORKERS):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = executor.map(lambda ip: connect_ip_host(ip, ports), network.hosts())
...
def tcp_stealth_scan(network, ports, max_workers= MAX_WORKERS):
    active_hosts = []
    print_banner(stealth=True)

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = executor.map(lambda ip: stealth_scan_ip(ip, ports), network.hosts())
