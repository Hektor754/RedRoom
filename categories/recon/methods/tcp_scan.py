...
class Handler:
    ...
    def tcp_scan(ip_range, tcp_flags, timeout, retries, filename, ftype, silent, max_workers=MAX_WORKERS):
        ...
        scan_func = SCAN_METHODS["stealth" if tcp_flags.stealth else "connect"]
        return scan_func(network, ports, timeout, retries, filename, ftype, silent, max_workers)
...
class TCPConnectScan:
    @staticmethod
    def scan(network, ports, timeout, retries, filename, ftype, silent, max_workers=MAX_WORKERS):
        active_hosts = []
        if not silent:
        ...
        for hostname, ip, active in results:
            ...
            if not silent:
                Output.print_host_result(hostname, ip, status)
            active_hosts.append({
                "hostname": hostname,
                "ip": ip,
                "status": status
