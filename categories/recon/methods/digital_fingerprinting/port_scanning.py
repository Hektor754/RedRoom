...
class PortScan:
    ...
    @staticmethod
    def Scan_method_handler(ip_range, tcp_flags=None, timeout=1, retries=1, max_workers=50):
        ...
        for scan_func in scan_funcs:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                res = list(executor.map(lambda ip: scan_func(ip, ports, timeout, retries), network.hosts()))

            formatted_results = []
            for item in res:
                ip = item[0]
                ports_found = item[1] if len(item) > 1 else []
            ...
            all_results[scan_name] = formatted_results
        ...
        for port in ports:
            try:
                for _ in range(retries):
                    ...
                    time.sleep(Utilities.randomize_time("ACK"))
            except Exception as e:
                print(f"[!] Error scanning {ip}:{port} - {e}")
